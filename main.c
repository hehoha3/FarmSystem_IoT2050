...
int main(int argc, char *argv[]) {
    ...
    struct mosquitto *mosq = mqtt_service_init(MQTT_HOST, MQTT_PORT, "iot2050_c_client");
    if (!mosq) {
        return -1;
    }
    ...
    modbus_t *ctx = modbus_service_init(device, baud, parity, data_bits, stop_bits, slave_id);
    if (!ctx) {
    ...
    */
    ...
    modbus_mapping_t *mb_mapping = modbus_service_mapping_init(HOLDING_REG_COUNT, 333);
    if (!mb_mapping) {
